"""Anti-parallel-path sensitivity version of the final r10_l5 tau/ell analysis.

This file is intentionally self-contained with respect to the NEW distance
definition. It does NOT modify main.py and does NOT overwrite the baseline
output directory.

Unchanged from the final baseline:
    radial resolution = 10
    lateral resolution = 5
    Earth prior variance = 0.01 (prior SD = 0.1)
    tau grid = 9 values
    ell grid = 8 values
    total hyperparameter grid = 72 points

Only the correlated-error ray-path kernel is changed to allow reversed
(entry <-> exit) endpoint matching. The same- and reversed-orientation
Gaussian kernels are group-averaged and normalized to unit diagonal.
"""
...
OUTPUT_DIR = Path(
    "outputs/tau_ell_posterior_r10_l5_prior_0p01_antiparallel_full72"
)
...
# ============================================================
# FINAL 72-POINT HYPERPARAMETER GRID
# ============================================================
# 9 tau values x 8 ell values = 72 grid points.
# This is intentionally identical to the final baseline run so that
# the only model change is the treatment of anti-parallel ray paths.
...
# ============================================================
# ANTI-PARALLEL-AWARE PATH DISTANCE
# ============================================================

def construct_antiparallel_path_distance_squared_matrices(
    ic_in: np.ndarray,
    ic_out: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Construct same- and reversed-orientation squared path distances.
    ...
    The two distances are retained separately.  They are combined at the
    KERNEL level, rather than by taking min(d_same^2, d_reverse^2).
    """

    d_same_squared = construct_path_distance_squared_matrix(
        ic_in,
        ic_out,
    )

    xyz_in = lonlatrad_to_xyz(ic_in)
    xyz_out = lonlatrad_to_xyz(ic_out)

    in_norms = np.sum(xyz_in**2, axis=1)
    out_norms = np.sum(xyz_out**2, axis=1)

    # cross_squared[i,j] = ||in_i - out_j||^2
    cross_squared = (
        in_norms[:, None]
        + out_norms[None, :]
        - 2.0 * (xyz_in @ xyz_out.T)
    )
    ...
    # Average the two cross-endpoint distances:
    # ||in_i-out_j||^2 and ||out_i-in_j||^2.
    d_reverse_squared = 0.5 * (
        cross_squared
        + cross_squared.T
    )
    ...
    d_same_squared = np.maximum(
        d_same_squared,
        0.0,
    )
    d_reverse_squared = np.maximum(
        d_reverse_squared,
        0.0,
    )
...
def construct_covariance(
    path_distance_same_squared: np.ndarray,
    path_distance_reverse_squared: np.ndarray,
    sigma: np.ndarray,
    tau: float,
    ell: float,
) -> np.ndarray:
    """Construct C_epsilon + C_delta using an orientation-invariant kernel.

    Let z_i=(in_i,out_i), and let R z_i=(out_i,in_i).  Starting from the
    ordered-path Gaussian kernel, average over the two possible relative
    orientations:

        K_group(i,j) = 0.5 * [K_same(i,j) + K_reverse(i,j)].

    This is the group-averaged kernel for endpoint reversal.  It is then
    normalized to unit diagonal,

        K_anti(i,j) =
            K_group(i,j) /
            sqrt(K_group(i,i) K_group(j,j)),

    so tau remains the marginal standard deviation of the correlated
    observational-error component.

    Finally:
        C_total = tau^2 K_anti + diag(sigma^2).
    """
    ...
    K_same = np.exp(
        -path_distance_same_squared
        / (2.0 * ell**2)
    )

    K_reverse = np.exp(
        -path_distance_reverse_squared
        / (2.0 * ell**2)
    )
    ...
    # Normalize to correlation form so diag(K_anti)=1.
    diagonal = np.diag(K_group).copy()

    if np.any(diagonal <= 0.0):
        raise RuntimeError(
            "Orientation-invariant kernel has a non-positive diagonal."
        )
    ...
    # Numerical symmetry and exact unit diagonal.
    K_anti = 0.5 * (
        K_anti
        + K_anti.T
    )
    ...
    covariance[diagonal_indices] += sigma**2
    ...
    # Keep this validation for the sensitivity run.
    try:
        np.linalg.cholesky(covariance)
    except np.linalg.LinAlgError as exc:
        raise RuntimeError(
            "Orientation-invariant total covariance unexpectedly failed "
            f"the positive-definiteness check for tau={tau}, ell={ell}."
        ) from exc
...
def main() -> None:
    ...
    logger.info(
        "Constructing anti-parallel-aware path-distance matrix..."
    )

    (
        path_distance_same_squared,
        path_distance_reverse_squared,
    ) = construct_antiparallel_path_distance_squared_matrices(
        ic_in,
        ic_out,
    )

    logger.info(
        "Same/reversed path-distance matrix shape: %s",
        path_distance_same_squared.shape,
    )

    # Diagnostic: how often does reversing the endpoint matching reduce
    # the path distance?  This does NOT mean all such pairs are genuinely
    # close anti-parallel paths; the distance thresholds below quantify that.
    upper = np.triu_indices(
        len(data),
        k=1,
    )

    same_distance = np.sqrt(
        path_distance_same_squared[upper]
    )
    reverse_distance = np.sqrt(
        path_distance_reverse_squared[upper]
    )

    reverse_is_closer = (
        reverse_distance < same_distance
    )

    print()
    print("ANTI-PARALLEL PATH-DISTANCE DIAGNOSTIC")
    print("--------------------------------------")
    print(
        "Pairs closer under reversed orientation =",
        int(np.sum(reverse_is_closer)),
        "/",
        reverse_is_closer.size,
        f"({100.0 * np.mean(reverse_is_closer):.6f}%)",
    )
    ...
    for threshold_km in (
        ...
        count = int(
            np.sum(
                reverse_distance <= threshold_km
            )
        )
        print(
            f"Reverse distance <= {threshold_km:6.1f} km: "
            f"{count:,} pairs "
            f"({100.0 * count / reverse_distance.size:.6f}%)"
        )

    # Free the 1-D diagnostic arrays before the expensive evidence loop.
    del same_distance
    del reverse_distance
    del reverse_is_closer
        ...
        for ell in ELL_GRID:
            ...
            covariance = (
                construct_covariance(
                    path_distance_same_squared=path_distance_same_squared,
                    path_distance_reverse_squared=path_distance_reverse_squared,
                    sigma=sigma,
                    tau=tau,
                    ell=ell,
                )
    ...
    print()
    print("9. ANTI-PARALLEL-AWARE PATH-DISTANCE MATRICES")
    print("-----------------------------------------------")

    for name, D2 in (
        ("same-orientation", path_distance_same_squared),
        ("reversed-orientation", path_distance_reverse_squared),
    ):
        print(f"{name} squared-distance matrix shape =", D2.shape)
        assert D2.shape == (len(data), len(data))
        print(f"{name} minimum squared distance =", np.min(D2))
        print(f"{name} maximum squared distance =", np.max(D2))
        print(
            f"{name} maximum asymmetry =",
            np.max(np.abs(D2 - D2.T)),
        )

    print(
        "Same-orientation maximum diagonal value =",
        np.max(np.abs(np.diag(path_distance_same_squared))),
    )
    print(
        "Reversed-orientation diagonal range =",
        np.min(np.diag(path_distance_reverse_squared)),
        "to",
        np.max(np.diag(path_distance_reverse_squared)),
    )
