...
# Completed 72-point PSD-safe anti-parallel hyperparameter run.
...
OUTPUT_DIR = Path(
    "outputs/earth_posterior_marginalized_r10_l5_prior_0p01_antiparallel_full72"
)
...
def construct_antiparallel_path_distance_squared_matrices(
    ...
    """Return same- and reversed-orientation squared path distances."""
...
def construct_antiparallel_covariance(
    ...
    """Construct the PSD-safe orientation-invariant total covariance.

    K_group = 0.5 * (K_same + K_reverse)

    K_anti(i,j) = K_group(i,j) /
                  sqrt(K_group(i,i) K_group(j,j))

    C_d = tau^2 K_anti + diag(sigma^2).

    This is the same covariance definition used in the completed
    anti-parallel tau/ell evidence calculation.
    """
    ...
    covariance[diagonal_indices] += sigma_squared
...
def load_and_validate_hyperparameter_posterior() -> pd.DataFrame:
    ...
    if not np.isclose(map_tau, 0.00475):
        raise RuntimeError(
            f"Unexpected MAP tau: {map_tau}. Expected 0.00475."
        )

    if not np.isclose(map_ell, 350.0):
        raise RuntimeError(
            f"Unexpected MAP ell: {map_ell}. Expected 350 km."
        )
    ...
    *,
    ...
    for ell, group in groups:
        for row in group.itertuples():
            ...
            covariance = construct_antiparallel_covariance(
...
def main() -> None:
    ...
    # Construct same- and reversed-orientation path distances once.
    logger.info(
        "Constructing same/reversed anti-parallel path-distance matrices..."
    )
    ...
    print()
    print("=" * 72)
    print("ANTI-PARALLEL EARTH-POSTERIOR MARGINALIZATION SUMMARY")
    print("=" * 72)
